...
* `plot_svm()`: Visualiza la frontera y resalta, mediante anillos amarillos, exclusivamente a los Vectores de Soporte ($\alpha_i > 0$).
...
* **Interpretación de los hallazgos:** La formulación dual prueba empíricamente la propiedad de **parsimonia** (esparsidad) de la SVM. Como se observa en los gráficos, la inmensa mayoría de los puntos de entrenamiento reciben un $\alpha = 0$, siendo completamente ignorados para el cálculo final del hiperplano. Únicamente los puntos resaltados en amarillo dictan la posición geométrica de la frontera.
...
Se demuestra que la solución que resuelve el problema mediante su modelo Dual depende estrictamente del producto punto de los vectores de entrada ($x_i^T x_j$), esta implementación sienta las bases matemáticas para la introducción de Funciones Kernel, permitiendo proyectar datos hacia dimensiones infinitas sin incrementar la complejidad computacional.
